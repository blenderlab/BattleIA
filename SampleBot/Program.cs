                    ...
                    if (result.Count > 0)
                    {
                        ...
                        switch ((Message)command)
                        {
                            ...
                            case Message.m_Respawn:
                                var respawned = new byte[1];
                                 respawned[0]= (byte)BotState.IsRespawned;
                                await client.SendAsync(new ArraySegment<byte>(respawned), WebSocketMessageType.Text, true, CancellationToken.None);
                                break;
                        }
                    } // if count > 1
                    else
